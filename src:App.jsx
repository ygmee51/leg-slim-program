...
const firebaseConfig = {
  apiKey: "AIzaSyAP9NOdLKIbp2WrbxA76CvrWtED3zIyj5A",
  authDomain: "leg-slim-program.firebaseapp.com",
  databaseURL: "https://leg-slim-program-default-rtdb.firebaseio.com",
  projectId: "leg-slim-program",
  storageBucket: "leg-slim-program.firebasestorage.app",
  messagingSenderId: "200963071938",
  appId: "1:200963071938:web:e53f51bbc30abb060d405b",
  measurementId: "G-1NHRMN8WRJ"
};
...
// shared data lives in collection "leg_slim_shared", personal data in "leg_slim_personal"
async function sGet(key, shared=false) {
  try {
    const col = shared ? "leg_slim_shared" : "leg_slim_personal";
    const safeKey = key.replace(/[/]/g, "_");
    const snap = await getDoc(doc(db, col, safeKey));
    if (!snap.exists()) return null;
    const data = snap.data();
    return data?.value !== undefined ? data.value : null;
  } catch (e) { console.log("sGet error:", e); return null; }
}
async function sSet(key, val, shared=false) {
  try {
    const col = shared ? "leg_slim_shared" : "leg_slim_personal";
    const safeKey = key.replace(/[/]/g, "_");
    await setDoc(doc(db, col, safeKey), { value: val, updatedAt: new Date().toISOString() });
  } catch (e) { console.log("sSet error:", e); }
}
